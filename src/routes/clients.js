...
router.get('/', auth, async (req, res, next) => {
  try {
    ...
    if (req.query.createdAfter) {
      filter.createdAt = { ...filter.createdAt, $gte: new Date(req.query.createdAfter) };
    }
    if (req.query.createdBefore) {
      filter.createdAt = { ...filter.createdAt, $lte: new Date(req.query.createdBefore) };
    }
    if (req.query.updatedAfter) {
      filter.updatedAt = { ...filter.updatedAt, $gte: new Date(req.query.updatedAfter) };
    }
    if (req.query.updatedBefore) {
      filter.updatedAt = { ...filter.updatedAt, $lte: new Date(req.query.updatedBefore) };
    }
    
    // Personal information filters
    if (req.query.personalFullName) {
      filter['personal.fullName'] = { $regex: req.query.personalFullName, $options: 'i' };
    }
    if (req.query.personalEmail) {
      filter['personal.email'] = { $regex: req.query.personalEmail, $options: 'i' };
    }
    if (req.query.personalPhone) {
      filter['personal.phone'] = { $regex: req.query.personalPhone, $options: 'i' };
    }
    if (req.query.personalPosition) {
      filter['personal.position'] = { $regex: req.query.personalPosition, $options: 'i' };
    }
    
    // Business information filters
    if (req.query.businessName) {
      filter['business.name'] = { $regex: req.query.businessName, $options: 'i' };
    }
    if (req.query.businessCategory) {
      filter['business.category'] = req.query.businessCategory;
    }
    if (req.query.businessDescription) {
      filter['business.description'] = { $regex: req.query.businessDescription, $options: 'i' };
    }
    if (req.query.businessAddress) {
      filter['business.mainOfficeAddress'] = { $regex: req.query.businessAddress, $options: 'i' };
    }
    ...
    if (req.query.businessPhone) {
      filter['contact.businessPhone'] = { $regex: req.query.businessPhone, $options: 'i' };
    }
    if (req.query.businessWhatsApp) {
      filter['contact.businessWhatsApp'] = { $regex: req.query.businessWhatsApp, $options: 'i' };
    }
    if (req.query.businessEmail) {
      filter['contact.businessEmail'] = { $regex: req.query.businessEmail, $options: 'i' };
    }
